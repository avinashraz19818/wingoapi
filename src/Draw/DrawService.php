...
class DrawService
{
     ...
     */
    public function history(GameDefinition $game, int $limit, int $offset = 0): array
    {
        $rows = $this->db->fetchAll(
            'SELECT * FROM ' . Tables::RESULTS . '
              WHERE game_code = ?
    ...
    public function countHistory(GameDefinition $game): int
    {
        return (int) $this->db->fetchValue(
            'SELECT COUNT(*) FROM ' . Tables::RESULTS . ' WHERE game_code = ?',
            [$game->code]
        );
    }
