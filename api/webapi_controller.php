...
function resolveGameCode(mixed $typeId, mixed $gameParam): string {
    if (!empty($gameParam)) {
        $g = trim((string)$gameParam);
        if (str_starts_with(strtolower($g), 'wingo_')) {
            $parts = explode('_', $g);
            return 'WinGo_' . strtoupper($parts[1] ?? '1M');
        }
        if (str_starts_with(strtolower($g), 'trxwingo_')) {
            $parts = explode('_', $g);
            return 'TrxWinGo_' . strtoupper($parts[1] ?? '1M');
        }
        if (str_starts_with(strtolower($g), '5d_') || str_starts_with(strtolower($g), 'd5_')) {
            $parts = explode('_', $g);
            return '5D_' . strtoupper($parts[1] ?? '1M');
        }
        if (str_starts_with(strtolower($g), 'k3_')) {
            $parts = explode('_', $g);
            return 'K3_' . strtoupper($parts[1] ?? '1M');
        }
        if (strcasecmp($g, '30s') === 0 || $g === '30') return 'WinGo_30S';
        if (strcasecmp($g, '1m') === 0 || $g === '1') return 'WinGo_1M';
        if (strcasecmp($g, '3m') === 0 || $g === '3') return 'WinGo_3M';
        if (strcasecmp($g, '5m') === 0 || $g === '5') return 'WinGo_5M';
        if (strcasecmp($g, '10m') === 0 || $g === '10') return 'WinGo_10M';
        return $g;
    }
...
$typeId = $payload['typeId'] ?? $payload['typeid'] ?? $_GET['typeid'] ?? $_GET['typeId'] ?? null;
$gameParam = $payload['gameCode'] ?? $payload['game_code'] ?? $payload['game'] ?? $_GET['gameCode'] ?? $_GET['game_code'] ?? $_GET['game'] ?? null;
$gameCode = resolveGameCode($typeId, $gameParam);
$pageSize = (int)($payload['pageSize'] ?? $payload['pagesize'] ?? $_GET['pageSize'] ?? $_GET['limit'] ?? 10);

$uri = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH);
$action = $_GET['action'] ?? $_POST['action'] ?? $payload['action'] ?? basename($uri);
$action = preg_replace('/\.(json|php)$/i', '', (string)$action);
...
switch (strtolower($action)) {
    ...
    case 'history':
        ...
        // $issue['issue_number'] is the round the client is showing. History is
        // filtered strictly below it so the Game History table stays one period
        // behind the header.
    ...
    case 'issue':
        $issue = $syncService->getCurrentIssue($gameCode);
        $startTs = (int)(strtotime($issue['start_time']) * 1000);
        $endTs = (int)(strtotime($issue['end_time']) * 1000);
        $intervalMin = (float)($issue['interval'] == 30 ? 0.5 : round($issue['interval'] / 60, 1));
        ...
        echo json_encode([
            ...
            'data' => [
                ...
                'countdown'         => (int)$issue['seconds_left'],
                'interval'          => (int)$issue['interval'],
                'intervalM'         => $intervalMin,
