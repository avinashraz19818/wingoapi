// Screenshot-reference fixture using the live native Wallet and TimeLeft DOM/CSS.
...
for(const width of [320,360,390,411,430]){
 ...
 assert.notEqual(await p.locator('.Wallet__C').evaluate(e=>getComputedStyle(e,'::before').display),'none','baseline must reproduce red layer');
 ...
 await p.screenshot({path:path.resolve('/home/user/inspection/timer32',`fixture-${width}.png`),fullPage:true});
 // Both home and unrelated games retain their original backgrounds.
 for(const route of ['/','/K3/K3_1M']){await p.evaluate(route=>history.pushState({},'',route),route);assert.equal(await p.locator('html').getAttribute('data-13l-wingo-ui'),'0');assert.notEqual(await p.locator('.Wallet__C').evaluate(e=>getComputedStyle(e,'::before').display),'none');}
 results.push({width,redLayer:data.redLayer,wallet:data.wallet,header:data.header,digit:data.digit,cell:data.cell,font:data.font,weight:data.weight,actualFonts,clipping:false,actionsPreserved:true,countdownPreserved:true,errors});await p.close();
}
await b.close();fs.writeFileSync(fixtures+'/results.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));})().catch(e=>{console.error(e);process.exit(1)});
