...
const addon36=JSON.parse(fs.readFileSync(root+'/tools/13l-native36.json','utf8'));assert.equal(hash(source),addon36.before_sha256);for(const pair of addon36.replacements){assert.equal(source.split(pair.old).length,2);source=source.replace(pair.old,pair.new);}assert.equal(hash(source),addon36.after_sha256);
...
// No .history/provides lookup in this fixture: the explicit native36 bridge must work.
await p.evaluate(()=>{const header=document.createElement('div');header.className='game-header';header.innerHTML='<div class="head-left"></div>';document.querySelector('main').prepend(header);});await run(1100);
assert.equal(await p.locator('.header-voice35').isEnabled(),true,'native36 sound bridge ready without DOM context lookup');await p.locator('.header-voice35').click();assert.equal(await p.evaluate(()=>native.soundEffects.value),true);assert.equal(await p.locator('.header-voice35').getAttribute('data-icon36'),'gamesaas_unmute');await p.locator('.header-voice35').click();assert.equal(await p.evaluate(()=>native.soundEffects.value),false);await p.evaluate(()=>app.unmount());assert.deepEqual(errors,[]);
