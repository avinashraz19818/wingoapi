...
| **Front-end compatibility** | `/api/Compat` answers an existing "AR style" client in its own dialect (`msg:"Succeed"`, `Color_green` bet strings, `premium`/`openCode` results) so a compiled front-end runs on this engine without a rebuild. |
...
748 assertions covering issue numbering, all five rule engines, the HMAC draw
...
- [`docs/COMPAT.md`](docs/COMPAT.md) — running an existing game front-end unchanged on this engine
