...
class UserController
{
    ...
    public static function adjustBalance(int $userId, string $type, float $amount, string $notes): array
    {
        ...
        try {
            ...
            // Admin adjustment is credited to both ledgers so the amount is
            // immediately visible in the Wingo game and in the main wallet.
            $oldWallet = (float) $user['wallet_balance'];
            $oldGame = (float) $user['game_balance'];
            $newWallet = $oldWallet + $amount;
            $newGame = $oldGame + $amount;

            if ($newWallet < 0 || $newGame < 0) {
                $pdo->rollBack();
            ...
            $stmt->execute([$newWallet, $newGame, $user['id']]);

            $stmt = $pdo->prepare("INSERT INTO wallet_logs (user_id, type, amount, balance_before, balance_after, notes) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$user['user_id'], $type . '_and_game', $amount, $type === 'game' ? $oldGame : $oldWallet, $type === 'game' ? $newGame : $newWallet, $notes . ' | synced to wallet and game balance']);

            $pdo->commit();
            return ['success' => true, 'message' => 'Wallet and game balance adjusted successfully', 'new_balance' => $type === 'game' ? $newGame : $newWallet, 'wallet_balance' => $newWallet, 'game_balance' => $newGame];
        } catch (Throwable $e) {
