...
function api_primary_user(): array
{
    ...
    if ($pdo && !empty($token)) {
        try {
            ...
            $user = $stmt->fetch();
            if ($user) {
                // Keep the game wallet available to Wingo. Older deployments
                // credited wallet_balance only, which made the game show zero.
                $wallet = (float) ($user['wallet_balance'] ?? 0);
                $game = (float) ($user['game_balance'] ?? 0);
                if ($wallet > $game) {
                    try {
                        $sync = $pdo->prepare("UPDATE api_users SET game_balance = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
                        $sync->execute([$wallet, $user['id']]);
                        $user['game_balance'] = $wallet;
                    } catch (Throwable $e) {}
                }
                $currentUser = $user;
                return $user;
    ...
    if ($pdo) {
        try {
            ...
            if ($row) {
                ...
                if ($wallet > (float) ($row['game_balance'] ?? 0)) {
                    try {
                        $sync = $pdo->prepare("UPDATE api_users SET game_balance = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
                        $sync->execute([$wallet, $row['id']]);
                        $row['game_balance'] = $wallet;
                    } catch (Throwable $e) {}
                }
                $currentUser = $row;
                return $row;
