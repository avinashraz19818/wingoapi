...
function api_ensure_schema(PDO $pdo): void
{
    ...
    if ($driver === 'mysql') {
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS api_users (
            ...
            game_balance DECIMAL(18,4) NOT NULL DEFAULT 0.0,
        ...
        $stmt = $pdo->prepare("INSERT IGNORE INTO api_users (user_id, username, nickname, phone, wallet_balance, game_balance, can_bet) VALUES (132257, 'local_member', 'MemberNNGKLPHA', '919119098026', 0, 0.0, 1)");
        $stmt->execute();
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS api_users (
            ...
            game_balance REAL NOT NULL DEFAULT 0.0,
        ...
        $stmt = $pdo->prepare("INSERT OR IGNORE INTO api_users (user_id, username, nickname, phone, wallet_balance, game_balance, can_bet) VALUES (132257, 'local_member', 'MemberNNGKLPHA', '919119098026', 0, 0.0, 1)");
        $stmt->execute();
    ...
    try {
        ...
        $stmt->execute(['https://dhaniwin1.club9.eu.cc']);
    } catch (Throwable $e) {
    }
...
function api_primary_user(): array
{
    ...
    // Some Wingo balance requests send token only inside a JSON body.
    if ($token === '') {
        $rawBody = (string) @file_get_contents('php://input');
        if ($rawBody !== '') {
            $jsonBody = json_decode($rawBody, true);
            if (is_array($jsonBody)) {
                $token = trim((string) ($jsonBody['token'] ?? $jsonBody['accessToken'] ?? $jsonBody['authorization'] ?? ''));
            }
        }
    }

    // Also accept the common login token cookie used by the web client.
    if ($token === '') {
        $token = trim((string) ($_COOKIE['ar_token'] ?? $_COOKIE['token'] ?? ''));
    }

    // Accept token header variants used by different Wingo builds.
    if ($token === '') {
        foreach (['HTTP_X_TOKEN', 'HTTP_X_ACCESS_TOKEN', 'HTTP_ACCESS_TOKEN', 'HTTP_TOKEN'] as $headerKey) {
            if (!empty($_SERVER[$headerKey])) {
                $token = trim((string) $_SERVER[$headerKey]);
                break;
            }
        }
    }
    ...
    // Never use another user's account as a fallback. If the token is
    // missing/invalid, return a zero-balance guest instead of exposing the
    // first database user balance to every UID.
    return [
        'id' => 1,
        'user_id' => 132257,
...
function api_share_copy_payload(): array
{
    ...
    $domain = (string) api_setting('share_domain', 'https://dhaniwin1.club9.eu.cc');
    if ($domain === 'https://dhaniwin7.com') {
        $domain = 'https://dhaniwin1.club9.eu.cc';
    }
    $host = (string) ($_SERVER['HTTP_HOST'] ?? '');
